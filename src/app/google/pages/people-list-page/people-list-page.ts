...
export class PeopleListPage {
  ...
  protected readonly filteredItems = computed(() => {
    ...
    return connections.filter((person) => {
      ...
      const email = person.emailAddresses?.some((e) => e.value.toLowerCase().includes(searchTerm)) || false;
      const phone = person.phoneNumbers?.some((p) => p.value.toLowerCase().includes(searchTerm)) || false;
